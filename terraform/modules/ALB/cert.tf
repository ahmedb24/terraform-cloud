...
resource "aws_route53_record" "toolingsite" {
  for_each = {
    for dvo in aws_acm_certificate.toolingsite.domain_validation_options : dvo.domain_name => {
...
resource "aws_acm_certificate_validation" "toolingsite" {
  certificate_arn         = aws_acm_certificate.toolingsite.arn
  validation_record_fqdns = [for record in aws_route53_record.toolingsite : record.fqdn]
}
